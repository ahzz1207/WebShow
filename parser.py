...
def close_br(h5_path):
    ...
    while i < length:
        ...
        if line[i] == "</tr>\n":
            ...
            i += 1
    print(line)
    open(h5_path, "w", encoding="utf-8").writelines(line)
...
def highlight(text, h5_pos):
    ...
    for index, poses in enumerate(h5_pos):
        ...
        for xy_ in poses:
            if len(xy_) == 6:
                h5_path = os.path.join(root_path, "draw", 'p' + str(xy_[5]) + '.html')
            elif len(xy_) == 9:
                h5_path = os.path.join(root_path, "draw", 'p' + str(xy_[8]) + '.html')
            close_br(h5_path)
